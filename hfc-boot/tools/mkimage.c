...
NXTARG:		;
	...
	debug("imagefile is %s,flag %x %x\n",params.imagefile,params.lflag,params.fflag);
	 ...
	 */
	debug("head size is %d\n",tparams->header_size); //512 byte 
	...
	if (!params.skipcpy) {
		if (params.type == IH_TYPE_MULTI ||
			...
			debug("mark copy file run copy %s\n",params.datafile);
   ...
   !defined(__APPLE__)
	...
	if (tparams->set_header) //image_set_header
