...
extern void tft_switch_fb_base_addr(unsigned int addr);
int fdt_tft_image(void *fdt, ulong tft_start, ulong tft_end, int force)
{
	int   nodeoffset;
	int   err, j, total;
	u32   tmp;
	...
	/* If there is no "chosen" node in the blob return */
	if (nodeoffset < 0) {
		printf("fdt_tft_image: %s\n", fdt_strerror(nodeoffset));
		return nodeoffset;
	}

	/* just return if tft_start/end aren't valid */
	if ((tft_start == 0) || (tft_end == 0))
		return 0;
	...
	err = fdt_add_mem_rsv(fdt, tft_start, tft_end - tft_start);
	if (err < 0) {
		printf("fdt_tft_image: %s\n", fdt_strerror(err));
		return err;
	}

	tft_switch_fb_base_addr(tft_start);

	path = fdt_getprop(fdt, nodeoffset, "linux,tft-start", NULL);
	if ((path == NULL) || force) {
		tmp = __cpu_to_be32(tft_start);
		err = fdt_setprop(fdt, nodeoffset,
			"linux,tft-start", &tmp, sizeof(tmp));
		if (err < 0) {
			printf("WARNING: "
				"could not set linux,tft-start %s.\n",
				fdt_strerror(err));
			return err;
		}
		tmp = __cpu_to_be32(tft_end);
		err = fdt_setprop(fdt, nodeoffset,
			"linux,tft-end", &tmp, sizeof(tmp));
		if (err < 0) {
			printf("WARNING: could not set linux,tft-end %s.\n",
				fdt_strerror(err));
