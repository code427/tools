...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_NONE:
		if (load == blob_start || load == image_start) {
			...
			printf("   Loading %s to %lx... ", type_name,image_start);
...
/*******************************************************************/
static int hfc_load_ram_disk(image_info_t *rd)
{
	ulong load = rd->load;
	ulong blob_start = rd->start;
	ulong blob_end = rd->end;
	ulong image_start = rd->image_start;
	ulong image_len = rd->image_len;
	ulong image_end = 0;
	__maybe_unused uint unc_len = 0x800000;
	
	int no_overlap = 0;

	const char *type_name = genimg_get_type_name(rd->type);
    ...
    flush_cache(load, (image_end - load) * sizeof(ulong));
	puts("OK\n");
	
	bootstage_mark(BOOTSTAGE_ID_KERNEL_LOADED);
		
	if(!no_overlap && (load < blob_end) && (image_end > blob_start))
	{
		return BOOTM_ERR_OVERLAP;
	}
...
#endif
	//arch_preboot_os(); //mx25pdk did not use

extern void boot_prep_linux(bootm_headers_t *images);
extern void boot_jump_linux(bootm_headers_t *images);
	boot_prep_linux(&images);
	boot_jump_linux(&images);
	//boot_fn(0, argc, argv, &images); //equel to do_bootm_linux();
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		printf("## Booting kernel from Legacy Image at %08lx ...\n",
				img_addr);
		hdr = image_get_kernel(img_addr, 0);//images->verify);
		if (!hdr)
			return NULL;
