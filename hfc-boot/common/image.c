...
static const table_entry_t uimage_type[] = {
	...
	{	IH_TYPE_RAMDISK,    "ramdisk",	  "RAM Disk Image",	},
	...
	*rd_end = 0;
	...
	if ((argc >= 3) && (strcmp(argv[2], "-") ==  0)) {
		...
		case IMAGE_FORMAT_LEGACY:
			...
			images->rd.type = image_get_type(rd_hdr);
			images->rd.comp = image_get_comp(rd_hdr);
			images->rd.os = image_get_os(rd_hdr);
			images->rd.start = (ulong)rd_hdr;
			images->rd.end = image_get_image_end(rd_hdr);
			images->rd.load = image_get_load(rd_hdr);
			images->rd.image_start = rd_data;
			images->rd.image_len = rd_data + rd_len;
...
/**
 * boot_tft_image_high - relocate tft image for kernel display
 * @lmb: pointer to lmb handle, will be used for memory mgmt
 * @tft_data: tft image data start address
 * @tft_len: tft image data length
 * @tft_start: pointer to a ulong variable, will hold final init tft image
 *      start address (after possible relocation)
 * @tft_end: pointer to a ulong variable, will hold final init tft image
 *      end address (after possible relocation)
 *
 * tft_start and tft_end are set to final (after relocation) tft image
 * start/end addresses if tft image start and len were provided,
 * otherwise set tft_start and tft_end set to zeros.
 *
 * returns:
 *      0 - success
 *     -1 - failure
		  ...
		  ulong *tft_start, ulong *tft_end)
{
	if (tft_data) {
		...
		*tft_start = (ulong)lmb_alloc_base(lmb,tft_len, 0x1000, 0xffffffff);
		if (*tft_start == 0) {
			puts("tft image - allocation error\n");
			goto error;
		}
		
		bootstage_mark(BOOTSTAGE_ID_COPY_TFT_IMAGE);

	
		*tft_end = *tft_start + tft_len;
		
		printf("   Loading TFT image to %08lx, end %08lx ... ",
				*tft_start, *tft_end);
	
		memmove_wd((void *)*tft_start, (void *)tft_data, tft_len, CHUNKSZ);
		puts("OK\n");
	...
	debug("   tft image load start = 0x%08lx, tft image load end = 0x%08lx\n",
			*tft_start, *tft_end);
