	...
	case USB_DESCRIPTOR_TYPE_HID:
		{
			...
			int class = 0;
			struct usb_class_hid_descriptor *hid_descriptor;//usb_class_descriptor *class_descriptor;

			if (hid_descriptor->bDescriptorType != USB_DT_HID) {
				dbg_ep0 (3, "[%d] interface is not HID", bNumInterface);
				return -1;
			}
			/* copy descriptor for this class */
			copy_config (urb, hid_descriptor, hid_descriptor->bLength, max);
#endif
		}
		break;
	case USB_DESCRIPTOR_TYPE_REPORT:
		{
			...
			struct usb_hid_report_descriptor *report_descriptor;
			if (!(report_descriptor = usbd_get_hid_report())) {
				serial_printf("Invalid string \n");
				return -1;
			}
			dbg_ep0(3, "hid_descriptor: %x length %d", report_descriptor->bDescriptorType, report_descriptor->wLength);
			copy_config (urb, report_descriptor->bData, report_descriptor->wLength, max);
#if 0
			//struct usb_class_report_descriptor *report_descriptor;
			
			//if (!(report_descriptor = usbd_get_hid_report())) {
			//	serial_printf("Invalid report descriptor\n");
			//	return -1;
			//}
			extern struct usb_class_report_descriptor *hid_report;

			serial_printf("type : %x\n",hid_report->bDescriptorType);
			serial_printf("len : %x\n",hid_report->wLength);
			serial_printf("len : %x\n",hid_report->bData[0]);

			if (hid_report->bDescriptorType != USB_DT_REPORT) {
				dbg_ep0 (3, "descriptor is not REPORT");
				return -1;
			}
			/* copy report descriptor for this class */
			copy_config(urb, hid_report->bData[0], hid_report->wLength, max); 
			/*if (max - urb->actual_length > 0) {
				int length = MIN (report_descriptor.wLength, max - urb->actual_length);
				memcpy (urb->buffer + urb->actual_length, &report_descriptor.bData[0], length);
				urb->actual_length += length;
			}*/
#endif
		}
...
int ep0_recv_setup (struct urb *urb)
{
	...
	if ((request->bmRequestType & USB_REQ_TYPE_MASK) != 0) {
		dbg_ep0(2,"device state:%d\n",device->device_state);
		if(device->device_state <= STATE_CONFIGURED){
			/*	Attempt to handle a CDC specific request if we are
			 *	in the configured state.
			 */
			return device->hid_recv_setup(request,urb);
		}
