...
static void _set_gpio_direction(u32 port, u32 index, int is_input)
{
	...
	u32 l=0;
	if(gpio_debug_info)  printf("LINE:%d REG[0X%08X] VAL[0X%08X] IDX=%d L[0X%08X] \n",__LINE__,reg,__REG(reg),index,l);	
	l = __REG(reg);
	if (is_input)
		l &= ~(1 << index);
	else
...
void mxc_set_gpio_direction(iomux_pin_name_t pin, int is_input)
{
	...
	if(gpio_debug_info)  printf("LINE:%d PORT=%d \n",__LINE__,port);	
...
static int _get_gpio_datain(u32 port, u32 index)
{
	u32 reg = gpio_port[port].base + DR;
	u32 l = 0;
...
int mxc_get_gpio_datain(iomux_pin_name_t pin)
{
	...
	port = GPIO_TO_PORT(gpio);
	return _get_gpio_datain(port, GPIO_TO_INDEX(gpio));
}


 void debug_gpio_config(u32 port, u32 index)
{
	gpio_debug_info = 1;
}
