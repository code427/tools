...
//	struct ccm_regs *ccm = (struct ccm_regs *)IMX_CCM_BASE;
	...
	printf("AHB clock:\t%s\n",strmhz(buf,imx_get_ahbclk()));
	printf("IPG clock:\t%s\n",strmhz(buf,imx_get_ipgclk()));
	
	printf("Reset cause: %s\n\n", get_reset_cause());
#if 0
printf("mpctl = %08x",readl(&ccm->mpctl));	/* Core PLL Control */
printf("upctl = %08x",readl(&ccm->upctl));	/* USB PLL Control */
printf("cctl = %08x",readl(&ccm->cctl));	/* Clock Control */
printf("cgr0 = %08x",readl(&ccm->cgr0));	/* Clock Gating Control 0 */
printf("cgr1 = %08x",readl(&ccm->cgr1));	/* Clock Gating Control 1 */
printf("cgr2 = %08x",readl(&ccm->cgr2));	/* Clock Gating Control 2 */
printf("pcdr[0] = %08x",readl(&ccm->pcdr[0]));	/* PER Clock Dividers */
printf("pcdr[1] = %08x",readl(&ccm->pcdr[1]));	/* PER Clock Dividers */
printf("pcdr[2] = %08x",readl(&ccm->pcdr[2]));	/* PER Clock Dividers */
printf("pcdr[3] = %08x",readl(&ccm->pcdr[3]));	/* PER Clock Dividers */
printf("rcsr = %08x",readl(&ccm->rcsr));	/* CCM Status */
printf("crdr = %08x",readl(&ccm->crdr));	/* CCM Reset and Debug */
printf("dcvr0 = %08x",readl(&ccm->dcvr0));	/* DPTC Comparator Value 0 */
printf("dcvr1 = %08x",readl(&ccm->dcvr1));	/* DPTC Comparator Value 1 */
printf("dcvr2 = %08x",readl(&ccm->dcvr2));	/* DPTC Comparator Value 2 */
printf("dcvr3 = %08x",readl(&ccm->dcvr3));	/* DPTC Comparator Value 3 */
printf("ltr0 = %08x",readl(&ccm->ltr0));	/* Load Tracking 0 */
printf("ltr1 = %08x",readl(&ccm->ltr1));	/* Load Tracking 1 */
printf("ltr2 = %08x",readl(&ccm->ltr2));	/* Load Tracking 2 */
printf("ltr3 = %08x",readl(&ccm->ltr3));	/* Load Tracking 3 */
printf("ltbr0 = %08x",readl(&ccm->ltbr0));	/* Load Tracking Buffer 0 */
printf("ltbr1 = %08x",readl(&ccm->ltbr1));	/* Load Tracking Buffer 1 */
printf("pcmr0 = %08x",readl(&ccm->pcmr0));	/* Power Management Control 0 */
printf("pcmr1 = %08x",readl(&ccm->pcmr1));	/* Power Management Control 1 */
printf("pcmr2 = %08x",readl(&ccm->pcmr2));	/* Power Management Control 2 */
printf("mcr = %08x",readl(&ccm->mcr));	/* Miscellaneous Control */
printf("lpimr0 = %08x",readl(&ccm->lpimr0));	/* Low Power Interrupt Mask 0 */
printf("lpimr1 = %08x",readl(&ccm->lpimr1));	/* Low Power Interrupt Mask 1 */
#endif
	return 0;
}
#endif
